# Anass El Rhaiti — Consultant Data

## 👋 About me
Data Engineer basé à **Bruxelles** avec 2 ans d'expérience en entreprise (Keolis, Ovalo).
Spécialisé dans la construction de pipelines ETL, la transformation de données avec dbt et l'orchestration avec Airflow.

## 🛠️ Tech Stack
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![dbt](https://img.shields.io/badge/dbt-FF694B?style=flat&logo=dbt&logoColor=white)
![Airflow](https://img.shields.io/badge/Airflow-017CEE?style=flat&logo=apacheairflow&logoColor=white)
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat&logo=docker&logoColor=white)
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-316192?style=flat&logo=postgresql&logoColor=white)
![SQL](https://img.shields.io/badge/SQL-4479A1?style=flat&logo=mysql&logoColor=white)

## 📂 Projects
| Projet | Description | Stack |
|--------|-------------|-------|
| [ETL Pipeline](https://github.com/anass53/data-engineering-etl-pipeline) | Pipeline complet Extract → Load → Orchestrate | Python · Airflow · Docker · PostgreSQL |
| [dbt Bootcamp](https://github.com/anass53/dbt-bootcamp) | Transformations dbt 3 couches + 19 tests | dbt · SQL · PostgreSQL |

## 📫 Contact
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=flat&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/anass-e-23197729b/)
MAIL: anasselrhaiti10@gmail.com
